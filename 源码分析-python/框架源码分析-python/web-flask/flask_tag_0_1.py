...
class _RequestContext(object):
    ...
    请求上下文包含所有请求相关的信息。在请求开始时创建，并推送到_request_ctx_stack，结束后在其末尾删除。
    将创建为WSGI环境提供的URL适配器和请求对象。
    """
...
def flash(message):
    ...
    向下一个请求闪烁一条消息。为了移除从会话中显示消息并将其显示给用户，
    模板必须调用:func: ' get_flashhed_messages
...
def get_flashed_messages():
    ...
    the same messages.
    从会话中提取所有闪现的消息并返回它们。在同一个请求中对该函数的进一步调用将返回相同的消息
    """
...
class Flask(object):
    ...
    def preprocess_request(self):
        ...
        在每个实际请求分发之前调用
        调用每个as:meth: ' before_request '修饰函数。
        如果这些函数中任何一个的返回值非None,就会被处理为视图函数的返回值,请求处理停止
        """
